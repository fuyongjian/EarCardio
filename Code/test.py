...
def test_model(model, test_loader, criterion, device, input_side, results_dir):
    model.eval()
    total_loss = 0
    with torch.no_grad():
...
def main():
    ...
    test_loss = test_model(model, test_loader, criterion, device, args.input_side, results_dir=args.results_dir)
