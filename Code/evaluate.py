...
def parse_args():
    parser = argparse.ArgumentParser(description='Evaluate a Transformer model for BCG to ECG/SCG prediction')
    parser.add_argument('--root_dir', type=str, default='./Dataset/testDataset/M4/', help='Dataset directory')
    parser.add_argument('--results_dir', type=str, default='./results/multiUsers/M4', help='Directory to save results and logs')
    ...
    parser.add_argument('--model_path', type=str, default='./results/input_length/0Sec/best_model.pth', help='Path to the saved model')
...
def evaluate_model(model, test_loader, criterion, device, input_side, results_dir):
    model.eval()
    total_loss = 0
    with torch.no_grad():
        for batch_idx, batch in enumerate(test_loader):
            ...
            loss = calculate_loss(output, target_signals[:,1:,:], target_length, criterion)
            total_loss += loss.item()

            save_all_results(output, target_signals[:,1:,:], target_length, phase='test', save_dir=os.path.join(results_dir, 'test'), batch_idx=batch_idx)
...
def main():
    ...
    test_loss = evaluate_model(model, test_loader, criterion, device, args.input_side, results_dir=args.results_dir)
